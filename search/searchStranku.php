...
$value = $_POST['value'];
echo '<ul>';
$query = $conn->getConnection()->query("SELECT stranka_id, ime, prezime FROM stranka WHERE ime LIKE '$value%' OR prezime LIKE '$value%'");
$result = $query or die($conn->getConnection()->error()); 
while ($run = $result->fetch_array()){
    $id = $run['stranka_id'];
    $ime = $run['ime'];
    $prezime = $run['prezime'];
    echo '<li><a class="a" href="../primke.php?action=nova_primka&stranka_id=' .$id. '">'.$ime.' '.$prezime.' </a></li>';
}
echo '</ul>';
}
?>